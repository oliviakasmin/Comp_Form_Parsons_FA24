...
class Turtle {
	...
	squigglyForward(distance) {
		let numSteps = distance / 10;
		const newX = this.x + cos(this.bearing) * distance;
		const newY = this.y + sin(this.bearing) * distance;
		...
		for (let i = 0; i <= numSteps; i++) {
			const stepX = lerp(this.x, newX, i / numSteps);
			const stepY = lerp(this.y, newY, i / numSteps);
			points.push({ x: stepX, y: stepY });
		}

		for (let i = 0; i < points.length - 1; i++) {
			const n = random(-3, 3);
			this.moveTo(points[i].x + n, points[i].y + n);
		}
	}

	dottedForward(distance) {
		let numSteps = distance / 5;
		const newX = this.x + cos(this.bearing) * distance;
		const newY = this.y + sin(this.bearing) * distance;
	...
	zigzagForward(distance) {
		...
		for (let i = 0; i < points.length - 1; i++) {
			let distanceStep = calculateDistance(
				points[i].x,
				points[i].y,
				points[i + 1].x,
				points[i + 1].y
